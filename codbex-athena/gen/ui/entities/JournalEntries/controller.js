angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', 'messageHub', 'entityApi', 'Extensions', function ($scope, messageHub, entityApi, Extensions) {
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				if (response.status != 200) {
					messageHub.showAlertError("JournalEntries", `Unable to count JournalEntries: '${response.message}'`);
					return;
				}
				if (response.data) {
				...
				request.then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("JournalEntries", `Unable to list/filter JournalEntries: '${response.message}'`);
						return;
					}
		...
		$scope.openDetails = function (entity) {
			$scope.selectedEntity = entity;
			messageHub.showDialogWindow("JournalEntries-details", {
				action: "select",
				entity: entity,
			});
		};
		...
		$scope.openFilter = function (entity) {
			messageHub.showDialogWindow("JournalEntries-filter", {
		...
		$scope.deleteEntity = function (entity) {
			...
			messageHub.showDialogAsync(
				...
				`Are you sure you want to delete JournalEntries? This action cannot be undone.`,
				[{
					id: "delete-btn-yes",
					type: "emphasized",
					label: "Yes",
				...
				if (msg.data === "delete-btn-yes") {
					entityApi.delete(id).then(function (response) {
						if (response.status != 204) {
							messageHub.showAlertError("JournalEntries", `Unable to delete JournalEntries: '${response.message}'`);
							return;
						}
						$scope.loadPage($scope.dataPage, $scope.filter);
